// ctx.ts — ctxEstimate 上下文估算公式 + 12 桶直方图 (契约冻结, 2026-09-22)
// 职责边界: 单一模块承载 ctxEstimate 的 provider 家族路由与桶边界表 — 这是跨端
...
// ctx 值 → 桶序号 0..11 (左开右闭; 边界值属下桶: 4096→0? 否 — (0,4k] 含 4k → 0)
export function ctxBucketIndex(ctx: number): number {
  for (let i = 0; i < CTX_BUCKET_EDGES.length; i++) {
    if (ctx <= CTX_BUCKET_EDGES[i]!) return i;
  }
  return CTX_BUCKET_COUNT - 1;
}
