...
// 覆盖: 三源探测 found/未发现报告、--harness 过滤、codex 日期路径剪枝 (整树跳过但
...
describe("collectAll 探测", () => {
  it("空 home → 三源全未发现, exit 语义由调用方判 records 空", async () => {
    ...
    try {
      const {results, statuses} = await collectAll({harnesses: [], sinceMs: null, home: h.home});
  ...
  it("--harness 单源过滤: 只探测指定源", async () => {
    const h = await makeHome();
    try {
      await writeLines(`${h.home}/.claude/projects/p/s.jsonl`, [claudeAssistant({msgId: "m", input: 1, ts: T0})]);
      const {results, statuses} = await collectAll({harnesses: ["claude-code"], sinceMs: null, home: h.home});
      expect(statuses.map((s) => s.harness)).toEqual(["claude-code"]);
  ...
  it("codex 日期路径剪枝: 窗口外整日目录跳过, 窗口内保留 (语义等价于逐文件过滤)", async () => {
    ...
    try {
      ...
      const {results, statuses} = await collectAll({harnesses: ["codex"], sinceMs: since, home: h.home});
      const codex = results.find((r) => r.harness === "codex")!;
  ...
  it("剪枝跨午夜回归: 会话落昨夜目录但最后 token_count 已进窗 → 不被误剪 (2026-09 review #2)", async () => {
    ...
    try {
      ...
      const {results} = await collectAll({harnesses: ["codex"], sinceMs: since, home: h.home});
      const codex = results.find((r) => r.harness === "codex")!;
  ...
  it("opencode 库损坏 → 该源 skipped 不拖垮 claude 源", async () => {
    ...
    try {
      ...
      const {results} = await collectAll({harnesses: [], sinceMs: null, home: h.home});
      const oc = results.find((r) => r.harness === "opencode")!;
      expect(oc.records).toHaveLength(0);
      expect(oc.skippedFiles.length).toBeGreaterThan(0);
      const cl = results.find((r) => r.harness === "claude-code")!;
...
describe("XDG_DATA_HOME 注入 (opencode 库位置)", () => {
  it("绝对路径 XDG_DATA_HOME 生效", async () => {
    ...
    try {
      ...
      const {statuses} = await collectAll({harnesses: ["opencode"], sinceMs: null, home: h.home});
