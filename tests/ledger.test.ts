...
import {CTX_HIST_COLS, MCTX_HIST_COLS, OUT_HIST_COLS} from "../src/ctx.js";

// 行直方图列求和 (Σ-不变式断言用 — 列名走 ctx.ts SSOT 勿手抄; 直方图列 NOT NULL
// DEFAULT 0 (ledger DDL), 索引访问 undefined 分支不可达, ! 锚定该契约)
const histSum = (r: Record<string, number>, cols: readonly string[]): number =>
  cols.reduce((a, c) => a + r[c]!, 0);
...
describe("Ledger 开库与版本守卫", () => {
  it("首开建表并落 schema_version; 重开复用", async () => {
    ...
    expect(l1.getMeta("schema_version")).toBe("5");
    l1.close();
...
describe("day_stats 重算", () => {
  ...
  it("ctx_hist 落盘损坏 → 发射时抛错 (不变量守卫)", async () => {
    ...
    db.exec("UPDATE day_stats SET ctx_32k_64k = ctx_32k_64k + 1"); // Σ=2 ≠ nReq=1 (外部改写模拟)
...
describe("ProfileV2 发射 (形状 + 不变量 + 双路径同构)", () => {
  ...
  it("out_hist 落盘损坏 (Σ≠nReq) → 发射时抛错; max_ctx_hist 按日聚合越界 → 抛错", async () => {
    ...
    const {createSqlite} = await import("../src/sqlite.js");
    const db = await createSqlite(join(home, "pricey-tokens", "usage.db"));
    db.exec("UPDATE day_stats SET out_0_32k = 0"); // Σ=0 ≠ nReq=1 (首桶清零)
    db.close();
    const l2 = await Ledger.open(home);
    ...
    db2.exec("UPDATE day_stats SET mctx_0_32k = 2"); // 2 会话贡献 > nSess=1
    db2.close();
...
describe("session_stats 物化 (主模型归因 + 轮次 join)", () => {
  ...
  it("跨日会话: 归因日 = last_ts 日; 后续请求迁日 → 旧归因日重算不残留 (会话原子性)", async () => {
    ...
    const d1Row = stats.find((r) => r.day === localDayKey(T)) as unknown as {n_turns: number; mctx_64k_128k: number};
    expect(d1Row.mctx_64k_128k).toBe(1); // d1 有 1 会话贡献 (100000 ∈ (64k,128k])
    ...
    const d1After = stats.find((r) => r.day === localDayKey(T)) as unknown as {mctx_64k_128k: number};
    const d2After = stats.find((r) => r.day === localDayKey(d2)) as unknown as {mctx_64k_128k: number; n_sess: number; n_req: number};
    expect(d1After.mctx_64k_128k).toBe(0); // d1 贡献迁走
    expect(d2After.mctx_64k_128k).toBe(1); // d2 接收
    expect(d2After.n_sess).toBe(1);
    expect(d2After.n_req).toBe(1);
    // 全局: 会话恰一增量 (两日直方图总和恒 1)
    const rows = stats as unknown as Array<Record<string, number>>;
    const total = rows.reduce((a, r) => a + histSum(r, MCTX_HIST_COLS), 0);
    expect(total).toBe(1);
    l.close();
  });
  ...
  it("day_stats 会话归并: n_turns 记 (归因日, 主模型); 多会话各自一增量", async () => {
    ...
    const glm = stats.find((r) => r.model === "zai/glm-5.3") as unknown as {n_turns: number; mctx_64k_128k: number; n_sess: number; n_req: number};
    // glm 行: sA 归因 (2 turns, 桶5 +1) + sB 的请求侧 (n_sess 2, n_req 2 — sB 两请求都算)
    expect(glm.n_turns).toBe(2);
    expect(glm.mctx_64k_128k).toBe(1); // (64k,128k] 会话最深上下文落桶
    expect(glm.n_sess).toBe(2);
    expect(glm.n_req).toBe(2);
    const cc = stats.find((r) => r.model === "claude-sonnet-5") as unknown as {n_turns: number; mctx_0_32k: number; n_sess: number; n_req: number};
    expect(cc.n_turns).toBe(1); // sB 归因 claude (主模型), 1 turn
    expect(cc.mctx_0_32k).toBe(1); // (0,32k] 会话最深上下文落桶
...
describe("v2 → v3 迁移 (自动回填)", () => {
  ...
  it("v2 库首开 → 迁移 v5 宽列 (区间命名) + 全量回填: session_stats 真实 / day_stats 独立列真实 / 存量 n_tools 与 n_turns 恒 0 / 二开幂等", async () => {
    ...
    const l = await Ledger.open(home);
    expect(l.getMeta("schema_version")).toBe("5");
    ...
    expect(histSum(m, CTX_HIST_COLS)).toBe(2); // ΣctxHist==nReq
    expect(histSum(m, OUT_HIST_COLS)).toBe(2); // ΣoutHist==nReq
    expect(m["n_tools"]).toBe(0); // 存量行 n_tools=0
    expect(m["n_turns"]).toBe(0); // 无 turn_events
    expect(histSum(m, MCTX_HIST_COLS)).toBe(2); // 会话数真实回填
